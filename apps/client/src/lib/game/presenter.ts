...
import { createEmptyCellGrid, applyBoatsToCells, applyShotsToCells } from '../grid/render-utils';
import type { CellState, Shot, ModifiableBoat } from '../grid/types';
...
function applyBoatsToGrid(boats: ModifiableBoat[]) {
	return (cells: CellState[][]) => boats.reduce(applyBoatsToCells(GRID_SIZE, markAsBoat), cells);
}

function applySunkBoatsToGrid(boats: ModifiableBoat[]) {
	return (cells: CellState[][]) => boats.reduce(applyBoatsToCells(GRID_SIZE, markAsSunk), cells);
}

function applyShotsToGrid(shots: Shot[]) {
	return (cells: CellState[][]) => shots.reduce(applyShotsToCells(GRID_SIZE), cells);
}
