...
export function applyBoatsToCells(
  size: number,
  transformFn: (cell: CellState) => CellState,
) {
...
export function applyShotsToCells(size: number) {
  return (cells: CellState[][], shot: Shot): CellState[][] =>
    applyShotToCells(cells, shot, size);
}
