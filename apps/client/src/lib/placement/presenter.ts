...
import type { PlacementState, CellState, Position, DragState, Boat } from '../grid/types';
...
export function renderPlacementCells(payload: RenderPlacementCellsPayload): CellState[][] {
	...
	const renderRowAtY = (y: number): CellState[] => R.times(renderCellAtPosition(y), size);
...
function renderCell(payload: RenderCellPayload): CellState {
	...
	const isValidPreview = isPreview && isPreviewValid(state, drag, draggedBoatId);
	const hasBoat = cell?.occupied === true && cell.boatId !== draggedBoatId;
	const isSelected = selectedBoatId !== null && cell?.boatId === selectedBoatId && !drag.isDragging;
