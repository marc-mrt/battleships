...
function parseEntity<Schema extends z.ZodType>(schema: Schema, entity: Record<string, unknown>) {
...
function createMappingError(error: unknown): UnexpectedDatabaseError {
	return new UnexpectedDatabaseError(`Failed to map entity to domain model: ${error}`);
}
